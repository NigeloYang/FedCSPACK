...
def check(config_path, train_path, test_path, args):
    # check existing dataset
    if os.path.exists(config_path):
        with open(config_path, 'r') as f:
            config = ujson.load(f)
        if config['num_clients'] ==  args.num_clients and \
            config['non_iid'] == args.niid and \
            config['balance'] == args.balance and \
            config['partition'] == args.partition and \
            config['alpha'] == args.alpha and \
...
def separate_data(data, num_classes, args):
        ...
        if partition == 'pat':
            ...
            for i in range(num_classes):
                ...
                if balance == 'balance':
                    ...
                    raise 'balance is balance/unbalance'
            ...
            raise 'IID: partition=pat -> balance'
        ...
        if partition == 'pat':
            ...
            for i in range(num_classes):
                ...
                if balance == 'balance':
                    ...
                    raise 'balance == balance or unbalance'
                num_samples.append(num_all_samples-sum(num_samples))
            ...
            print('*'*50,' start generate exdir data ','*'*50)
            ...
            print('*'*50,' The second level: allocate data idx ','*'*50)
            dataidx_map = {}
            ...
            while min_size < min_require_size:
                ...
                for k in range(K):
                    ...
                    if balance == 'balance':
                        ...
                        proportions = np.array([p * (j in clientidx_map[k]) for j, (p, idx_j) in enumerate(zip(proportions, idx_batch))])
                    proportions = proportions / proportions.sum()
                    proportions = (np.cumsum(proportions) * len(idx_k)).astype(int)[:-1]
            ...
            raise 'noniid: partition=pat -> balance; partition=dir -> No balance; partition=exdir -> balance;'
    
    else:
        raise 'niid = iid or noniid'
...
def save_file(config_path, train_path, test_path, train_data, test_data, num_classes, statistic, args):
    config = {
        'num_clients': args.num_clients, 
