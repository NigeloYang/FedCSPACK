...
if __name__ == '__main__':
    print('\n', '*' * 50, 'ResNet 18', '*' * 50, '\n')
    emnist = resnet18(num_classes=62, in_dim=1)
    for name, parameter in emnist.named_parameters():
        print(f'name: {name} --- parameter size: {parameter.size()}')
    summary(emnist, input_size=(16, 1, 28, 28))

    print('\n', '*' * 50, 'ResNet 18', '*' * 50, '\n')
    cifar100 = resnet34(num_classes=100, in_dim=3)
    for name, parameter in cifar100.named_parameters():
        print(f'name: {name} --- parameter size: {parameter.size()}')
    summary(cifar100, input_size=(16, 3, 32, 32))